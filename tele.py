...
def chunkify(list, chunkSize=15):
    chunks = []
    count = 0
    tempList = []
    for list1 in list:
...
def addUsersToChannel(client, users, channel):
    ...
    for usersToAdd1 in usersToAdd:       
        ...
        if count >= 50:
            ...
            print('adding has been paused for '+str(tick)+' seconds in line with Telegram guidelines, it will resume immediately the time elapses.')
            sleep(tick)
            count = 0
            continue
    count = count + 1
...
def work(peters, getFrom=[], filepath='users.txt'):
    """
    This method stores users from channels channels specified by the getFrom param avove to a given file.
    """
    # do()
